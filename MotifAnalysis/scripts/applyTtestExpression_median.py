...
with open(exprFile,'r') as exprFile:
    ...
    for line in exprFile:
        ...
        expr[FBID] = np.median([float(ZT0),float(ZT4),float(ZT8),float(ZT12),float(ZT16),float(ZT20),float(ZT24),float(ZT28),float(ZT32),float(ZT36),float(ZT40),float(ZT44)])
exprFile.close()
...
for motif in sigInfo:
    ...
    outfile = open('results_medianExpressionTtest_' + motif[0] + '_' + outbase + '_' + group + '.txt', 'w')  # can't have motif symbol in outfile name bc some have ()
