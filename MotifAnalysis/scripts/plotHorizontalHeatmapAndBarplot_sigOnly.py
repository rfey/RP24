# rmf 3.25.2020, last modified 9.20.2021
# cleaned up 3.1.2022
...
# USAGE
usage = 'python ' + sys.argv[0] + ' <summary file> <q-value threshold>'
...
def readSummaryFile(summaryFile):
    ...
    for line in summaryFile:
        ...
        # take care of NO_TEST cases for qval
        ...
        duplicateCheck[motifBase].append((GOIgroup, motifDBID, motifSymbol, qval))
        ...
        if (GOIgroup, motifSymbol) not in motifSymbolDict:
            if motifDBID not in plotInfo[GOIgroup]:
                plotInfo[GOIgroup][motifDBID] = float(qval)
...
qThresh = float(sys.argv[2])  # for determining what appears on the heatmap

outbase = summaryFile.replace('motifEnrichmentSummary_','').replace('.txt','') + '_qThresh' + str(qThresh)
...
for GOIgroup in plotInfo:
    for motifDBID in plotInfo[GOIgroup]:
        qval = plotInfo[GOIgroup][motifDBID]
        ...
        if qval <= qThresh:
            ...
            if GOIgroup not in filtered:
                filtered[GOIgroup] = {}
            filtered[GOIgroup][motifDBID] = qval
...
# fill plotting matrix using original qval dict in case some cells don't pass thresh but need to be filled
for i in range(len(clusterNames)):
    for j in range(len(filteredTFs)):
        # plotting qval
        qval = plotInfo[clusterNames[i]][filteredTFs[j]]
        plottingMatrix[i][j] = qval
...
plot.yticks(np.arange(len(clusterNames)), clusterLabels, ha='right', va = 'center', rotation_mode = 'anchor', fontsize = 20) 
...
ax1.set_xticklabels(xlabels, rotation = 90, fontsize = 16)
...
outFile = 'horizontalHeatmapBarplot_' + outbase + '.pdf'
