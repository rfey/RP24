...
with open(exprFile,'r') as exprFile:
    ...
    for line in exprFile:
        ...
        expr[FBID] = np.mean([float(ZT0),float(ZT4),float(ZT8),float(ZT12),float(ZT16),float(ZT20),float(ZT24),float(ZT28),float(ZT32),float(ZT36),float(ZT40),float(ZT44)])
exprFile.close()
print 'Read in mean expression for', len(expr), 'transcripts.'
...
for motif in sigInfo:
    ...
    expr_sig = getMedian(expr, sig)  # returns a dictionary
    expr_notsig = getMedian(expr, notSig)

    pval = testSignificance(expr_sig.values(), expr_notsig.values())

    # initiate outfile and write header with pval and motif info
    outfile = open('results_meanExpressionTtest_' + motif[0] + '_' + outbase + '_' + group + '.txt', 'w')  # can't have motif symbol in outfile name bc some have ()
    ...
    for ID in expr_sig:
        outfile.write(ID[0] + '\t' + ID[1] + '\t' + str(expr_sig[ID]) + '\tyes\n')
    for ID in expr_notsig:
        outfile.write(ID[0] + '\t' + ID[1] + '\t' + str(expr_notsig[ID]) + '\tno\n')
    outfile.close()
