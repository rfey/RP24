...
outbase = resultsFile.split('Ttest_')[1].replace('.txt','')
print outbase 
...
if 'Cluster' in resultsFile:
    ...
    clusterName = outbase.replace('results_', '')
print clusterName
...
with open(resultsFile,'r') as resultsFile:
    ...
    for line in resultsFile:
        transcriptID, transcriptSymbol, val, sig = line.strip().split('\t')  # sig = yes or no
        if sig == 'yes':
            sigList.append(float(val))
        elif sig == 'no':
            notsigList.append(float(val))
resultsFile.close()

outfile = 'boxplot_' + ylabel + '_' + outbase + '.pdf'
